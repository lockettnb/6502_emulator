...
//  unsigned char *opcode = &memory[addr];
 int  op_size=1;                // assume one byte instruction or data byte
 char op_text[16];              // text for the opertion code including any address modes
 char address_text[16];
 char *p;                       // temporary pointer, used to scan instruction

 regex_t   *dbreg;              // compiled regular expressions
...
// make sure the disassemble text line is empty
    strcpy(dline, "");
...
// compile regex to match a data byte
    rc=regcomp(dbreg, r_data_byte, REG_EXTENDED);
    if (rc !=0) {
        fputs("Error: problem compiling databyte regex", stderr);
        exit(1);
    }

    strcpy(op_text, opcodes[(int) mread(addr)]);
    rc = regexec(dbreg, op_text, 1, matches, 0); // check if this is a data byte

    if(rc == 0) {             // match found, this is a byte of data not opcode
        sprintf(op_text, ".db $%02x", mread(addr));
    } else {                  // must be a valid instruction opcode
        p=op_text;            // scan op text looking for n's
        while(*p != '\0')
            if(*p++ == 'n') op_size++;
        ...
        if(op_size==3)
            sprintf(address_text, "$%02x%02x", mread(addr+2), mread(addr+1));
    ...
    switch (op_size) {
        case 1: 
            sprintf(dline, "%04x  %02x        %s", addr, mread(addr+0), op_text);
            break;
        case 2: 
            sprintf(dline, "%04x  %02x %02x     %s", addr, mread(addr+0), mread(addr+1),op_text); 
            break;
        case 3: 
            sprintf(dline, "%04x  %02x %02x %02x  %s", addr, mread(addr+0), mread(addr+1), mread(addr+2),op_text);
            break;
    }
    return(op_size);
}
